RSpec.describe Munificent::DonatorBundleAssignmentJob do
  describe "#perform" do
    ...
    it "assigns bundles only for active fundraisers" do
      ...
      expect(Munificent::DonatorBundleAssigner).to have_received(:assign).once
    end
    ...
    context "if there's no funds for a fundraiser" do
      ...
      it "does not assign any bundles" do
        perform
        expect(Munificent::DonatorBundleAssigner).not_to have_received(:assign)
      end
    end
