RSpec.describe Munificent::DonatorBundleAssigner do
  describe ".assign(donator:, bundle:, fund:)" do
    ...
    let(:bundle_tiers) do
      [
        build("munificent_bundle_tier", price: top_tier_price),
        build("munificent_bundle_tier", price: middle_tier_price),
        build("munificent_bundle_tier", price: bottom_tier_price),
      ]
    end
    ...
    context "when the donations are enough for part of the bundle" do
      ...
      context "and the bundle is already assigned" do
        let(:existing_donator_bundle) { Munificent::DonatorBundle.create_from(bundle, donator:) }
    ...
    context "when there's a little bit more than enough for the whole bundle" do
      ...
      context "and the fundraiser is in pro bono mode" do
        before do
          bundle.fundraiser.update(overpayment_mode: Munificent::Fundraiser::PRO_BONO)
        end
      ...
      context "and the fundraiser is in pro se mode" do
        before do
          bundle.fundraiser.update(overpayment_mode: Munificent::Fundraiser::PRO_SE)
        end
