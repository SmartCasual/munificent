RSpec.describe Munificent::Payment do
    ...
    context "with a Stripe payment" do
      ...
      context "when a payment already exists with that Stripe ID" do
        ...
        it "runs the assignment job on the existing payment" do
          ...
          expect(Munificent::PaymentAssignmentJob).to have_been_enqueued.with(existing_payment.id, provider: :stripe)
        end
      end
      ...
      context "when a payment already exists with a different Stripe ID" do
        ...
        it "runs the assignment job on a new payment" do
          ...
          expect(Munificent::PaymentAssignmentJob).to have_been_enqueued
          expect(Munificent::PaymentAssignmentJob).not_to have_been_enqueued.with(existing_payment.id, anything)
        end
      end
    end
    ...
    context "with a PayPal payment" do
      ...
      context "when a payment already exists with that PayPal ID" do
        ...
        it "runs the assignment job on the existing payment" do
          ...
          expect(Munificent::PaymentAssignmentJob).to have_been_enqueued.with(existing_payment.id, provider: :paypal)
        end
      end
