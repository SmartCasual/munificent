RSpec.describe Munificent::DonatorBundleTier, type: :model do
  ...
  describe "triggering bundle fulfillment" do
    ...
    before do
      allow(Munificent::KeyAssignment::RequestProcessor).to receive(:queue_fulfillment)
    end

    it "triggers fulfillment when the tier is unlocked" do
      tier.unlock!
      expect(Munificent::KeyAssignment::RequestProcessor).to have_received(:queue_fulfillment).with(tier)
    end
    ...
    it "does not trigger fulfillment when the tier is updated to locked" do
      ...
      expect(Munificent::KeyAssignment::RequestProcessor).not_to have_received(:queue_fulfillment)
    end
    ...
    it "triggers fulfillment when the tier is created as unlocked" do
      ...
      expect(Munificent::KeyAssignment::RequestProcessor).to have_received(:queue_fulfillment).with(new_tier)
    end
  ...
  describe "#fulfilled?" do
    it "returns true if the donator bundle tier is fulfilled" do
      ...
      tier.bundle_tier.games.each do |game|
        tier.keys << build("munificent_key", game:)
      end
